...
public class Main {
	...
	static int Tetromino(int[][] mat, int i, int j)
	{
		...
		int r;
		// ㅜ 자 형태
		if(0<=i+1 && i+1<mat.length && 0<= j-1 && j-1 < mat[i].length && 0<= j+1 && j+1 < mat[i].length){
			// 1 (ㅗ)
			r = mat[i][j] + mat[i+1][j-1] + mat[i+1][j] + mat[i+1][j+1];
			
			res = res>r?res:r;
		}
		if(0<=i+1 && i+1<mat.length && 0<=i+2 && i+2<mat.length && 0<= j+1 && j+1 < mat[i].length)
		{
			// 2 (ㅏ)
		...
		if(0<=i+1 && i+1<mat.length && 0<=j+2 && j+2<mat[i].length && 0<= j+1 && j+1 < mat[i].length)
		{
			// 4 (ㅜ)
		...
		if(0<= j+1 && j+1 < mat[i].length && 0<= j+2 && j+2 < mat[i].length && 0<= j+3 && j+3 < mat[i].length)
		{
			// 18(세로 우상 번개)
	...
	static int Tetromino(int[][] mat, boolean[][] chk, int i, int j)
	{
		...
		for(int k = 0;k<3;k++)
		{
			...
			for(int d=0;d<4;d++)
			{
				if(0>n_i+di[d]||n_i+di[d]>=mat.length||
					0>n_j+dj[d]||n_j+dj[d]>=mat[0].length) continue;
